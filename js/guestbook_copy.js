window.addEventListener("DOMContentLoaded", (event) => {
  ...
  function guestbook(memberindex) {
    ...
    document.querySelector(".sendinfo").addEventListener("click", () => {
      ...
      if (support.length > 0) {
        board.innerHTML = makeCommentView(support);
      } else {
      ...
      location.reload();
      if (data.length > 0) {
        board.innerHTML = makeCommentView();
      } else {
        let thankyou = `<td id="thankyoutext" style="width:1300px">응원 감사합니다!</td>`;
        board.innerHTML = thankyou;
      }
      ...
      removeimg.forEach((item, index) => {
        item.addEventListener("click", () => {
          ...
          console.log(support);
          removeComment(memberindex, index, support);
    ...
    window.addEventListener("load", () => {
      ...
      if (supportData) {
        //로컬스토리지에 supportData가 있으면 아래코드실행
        support = JSON.parse(supportData);

        makeCommentView(memberindex);
        bindRemoveEvent(memberindex, support);
        bindEditEvent(memberindex, support);
      }
    });
  }
});
...
function removeComment(memberindex, index, support) {
...
function makeEditInput() {
  ...
  tr.querySelector(
    ".changevalue"
  ).innerHTML = `<input class='editinvalue' type="text" value="${currentValue}">
...
function makeEditCompleteButton() {
  ...
  completionButton.addEventListener("click", () => {
    ...
    tr.querySelector(
      ".changevalue"
    ).innerHTML = `<td class='changevalue'>${currentValue}</td>`;
  });
}

function bindRemoveEvent(memberindex, support) {
  let removeimg = document.querySelectorAll(".removeimg");
  removeimg.forEach((item, index) => {
    item.addEventListener("click", () => {
      support.splice(index, 1);
...
function bindEditEvent(memberindex, support) {
  let editimg = document.querySelectorAll(".editimg");
  editimg.forEach((item, index) => {
    item.addEventListener("click", () => {
      let tr = item.closest("tr");
